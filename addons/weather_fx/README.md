...
The ambience is a `BiomeAmbience` resource per kind of place: the biomes it belongs to and a loop for each of clear, rain and storm by day and by night (`stream_for(weather, is_day)`; blue sky, cloudy and snow share the clear loop). `WeatherFX.bgs_sets` holds the sets a level has; the first whose `biomes` include the current biome plays, a biome no set covers plays `WeatherFX.bgs_default`, and nothing when that is empty too. `weather_fx.tscn` ships `forest.tres` (the wooded biomes: Shadow Woods, Misty Woods, Ancient Forest, Tropical Rainforest) and `beach.tres` (Coastal Plains, Humid Coast). `sea.tres`, `cave.tres` and `interior.tres` are there for a game to put on a boat, in a cave or indoors, and belong to no biome. The plains and the deserts are quiet until a game gives them a set. `WeatherAudio` makes one player per loop at ready and never restarts the one that stays the target, so a change of weather within the same loop is seamless.
