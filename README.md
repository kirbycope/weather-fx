...
> All core scripts register standalone global class names (`WeatherFX`, `WeatherZone`, `WeatherForecastDisplay`, `TemperatureGaugeDisplay`, `WindDirectionDial`, `WindVFX`, `FallingLeaves`, `GrassField`, `ClimateData`, `WaterRipples`).
...
- **Instanced Grass Generator (`GrassField`)**: High-performance `MultiMeshInstance3D` field generator supporting Quaternius stylized grass models (`Common Short`, `Common Tall`, `Wispy Short`, `Wispy Tall`) and custom meshes. `ignite_at(world_pos, radius, duration)` starts a wildfire that spreads as a cellular front on the field's 2 m origin grid: lit cells pass the fire to their neighbours at the BotW 1.2-1.8 m/s creep speed (`fire_spread_speed`), slower across and against the wind, so it grows as a ragged ring leaning downwind; each blade chars through the grass shader (`grass_wind.gdshader` reads the blade's MultiMesh custom data against the material's `fire_clock`), the front stops growing after `duration`, cells stay ash, and rain douses it (nothing catches while it rains). `douse_at(world_pos, radius)` puts out just the cells and flames within reach (a water spell) and leaves the rest of the front burning. A flame more than `IGNITE_HEIGHT` (1 m) above or below the blades lights nothing, so a torch on a platform over the grass is safe.
